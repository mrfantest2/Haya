...
final class RankRecognizer {
    private final TemplateStore store;

    RankRecognizer(TemplateStore store) { this.store = store; }
    ...
    SymbolPrediction<Integer> recognize(Mat indexCrop) {
        ...
        Mat query = TemplateStore.normalizeGlyph(region, 64, 80);
        region.release();
        if (Core.countNonZero(query) < 20) { query.release(); return null; }

        Integer bestValue = null;
        double best = -1;
        for (TemplateStore.Template<Integer> template : store.rankTemplates()) {
    ...
    private static double confidence(double similarity) {
        return Math.max(0.0, Math.min(1.0, .32 + .68 * similarity));
    }
}
